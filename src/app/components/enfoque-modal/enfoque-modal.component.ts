...
@Component({
  selector: 'app-enfoque-modal',
  templateUrl: './enfoque-modal.component.html',
  styleUrls: ['./enfoque-modal.component.scss']
})
export class EnfoqueModalComponent implements OnInit {
    ...
    if (this.isValid()) {
      ...
      if (this.mode === 'Editar') {
        ...
        const request: JSON = {
          enfoque:
          {
            enfoqueId: categoria.enfoqueId,
            enfoqueDescription: categoria.descripcion,
            estado: categoria.estado
          }
        } as unknown as JSON;
        this.enfoqueService.update(request, categoria.enfoqueId)
          .then(response => {
            this.createEdit.emit();
            this.eventMantenedorEnfoqueService.refreshTable();
            this.loading = false;
        ...
        this.enfoqueService.create(request)
          .then(response => {
            this.createEdit.emit();
            this.eventMantenedorEnfoqueService.refreshTable();
            this.loading = false;
